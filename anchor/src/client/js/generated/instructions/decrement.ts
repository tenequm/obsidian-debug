...
export type DecrementInstruction<
  ...
  TAccountObsidianprotocol extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountObsidianprotocol extends string
        ? WritableAccount<TAccountObsidianprotocol>
        : TAccountObsidianprotocol,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountObsidianprotocol extends string = string> = {
  obsidianprotocol: Address<TAccountObsidianprotocol>;
};
...
>(
  input: DecrementInput<TAccountObsidianprotocol>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountObsidianprotocol> {
  // Program address.
  const programAddress = config?.programAddress ?? OBSIDIANPROTOCOL_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    obsidianprotocol: { value: input.obsidianprotocol ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.obsidianprotocol)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountObsidianprotocol>;
...
> = {
  ...
  accounts: {
    obsidianprotocol: TAccountMetas[0];
  };
...
>(
  ...
  return {
    ...
    accounts: {
      obsidianprotocol: getNextAccount(),
    },
