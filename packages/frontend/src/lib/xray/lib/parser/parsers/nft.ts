...
function returnCompressedNftEventArray(
  value: CompressedNftEvent | CompressedNftEvent[] | null
) {
  ...
  for (let i = 0; i < nftEvent.length; i++) {
    ...
    if (!address) {
      actions.push({
        ...
        amount: 1,
        from: "",
        sent: nftEventAction.assetId ?? undefined,
      ...
      actions.push({
        ...
        amount: 1,
        from: "",
        received: nftEventAction.assetId ?? undefined,
        to: nftEventAction.newLeafOwner || "",
      });
    } else {
      actions.push({
...
export const parseCompressedNftBurn: ProtonParser = (transaction, address) => {
  ...
  for (let i = 0; i < nftEvent.length; i++) {
    ...
    } else if (nftEventAction.type === TransactionType.COMPRESSED_NFT_MINT) {
      actions.push({
