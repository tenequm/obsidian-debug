...
const TOKEN_PROGRAM_ERRORS: ErrorPattern[] = [
  {
    ...
    explanation:
      "The account does not have enough tokens to complete the operation",
    commonCauses: [
      "Trying to transfer more tokens than available",
...
const SYSTEM_PROGRAM_ERRORS: ErrorPattern[] = [
  {
    ...
    commonCauses: [
      "Trying to create account that already exists",
...
const DEX_ERRORS: ErrorPattern[] = [
  {
    ...
    explanation:
      "Price moved beyond the specified slippage tolerance during swap",
    commonCauses: [
      "High market volatility",
      "Slippage tolerance set too low",
      "Large trade size for liquidity pool",
    ],
    suggestedFixes: [
...
export function findErrorsByProgram(programName: string): ErrorPattern[] {
  return ERROR_PATTERNS.filter((pattern) =>
    pattern.program.toLowerCase().includes(programName.toLowerCase())
  );
}
